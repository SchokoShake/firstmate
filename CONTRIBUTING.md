...
tests/fm-logbook.test.sh                  # logbook client: config resolution and URL-derived port, push/sync/resolve/ack dry-run and live bearer posts (no token leak), safe-slug id guard, detached-launcher inert-vs-healthy no-ops, the attention-set compose/refresh baseline (inert-when-off, {projects,items} classification, dry-run and live sync), the inbound board-response poll (opted-out no-op, 204-silent, pending-stash-and-print, rate-limited error), and bootstrap opt-in drop / opt-out remove of the poll shim + 15s cadence plus the captain-facing board link and auto-sync
