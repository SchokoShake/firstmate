...
- **Explicit project modes** - each project ships via `no-mistakes`, `direct-PR`, or `local-only`, with an optional `+yolo` autonomy flag and an optional `+captain-merge` flag for work only you may merge.
...
- **Optional logbook** - opt in with one local `config/logbook.env` flag to feed a loopback-only "what needs you" attention board: firstmate hands you the board link and auto-syncs it from fleet state at session start, keeps it current as work is dispatched, finished, and decided, and pushes every pending decision, ready action, and FYI onto one glanceable surface the moment it arises, and you answer a card on the board (or still in chat) and firstmate acts on it and clears the card - the answer-loop, the board's liveness, and that ongoing refresh all ride the existing watcher check rail like X mode, changing no non-logbook behavior, and a dry-run preview records would-be calls locally.
...
Full architecture - the supervision engine, worktree isolation, secondmates, dispatch profiles, project modes, optional X mode, the optional logbook attention board, fleet sync, and self-update - is in [docs/architecture.md](docs/architecture.md).
...
- [docs/configuration.md](docs/configuration.md) - environment variables, `FM_HOME`, runtime backend selection, optional X mode, the optional logbook board, the files you set, and harness support.
