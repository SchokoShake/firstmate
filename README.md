...
- **Optional logbook** - opt in with one local `config/logbook.env` flag to feed a loopback-only "what needs you" attention board: firstmate hands you the board link and auto-syncs it from fleet state at session start, then pushes every pending decision, ready action, and FYI onto one glanceable surface the moment it arises, and you answer a card on the board (or still in chat) and firstmate acts on it and clears the card - the inbound answer-loop rides the existing watcher check rail like X mode, changing no non-logbook behavior, and a dry-run preview records would-be calls locally.
