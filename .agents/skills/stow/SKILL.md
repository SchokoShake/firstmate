...
2. Propose pinned relocation only.
   For a pinned candidate, append a `proposed-offload` section with the same fields to the completion receipt and create or refresh one durable captain-held backlog item using `tasks-axi add`, `bin/fm-captain-hold.sh <id> --reason "<reason>"`, `tasks-axi show <id> --full`, and `tasks-axi update <id> --body-file <path>` as appropriate.
   Hold it through that script rather than `tasks-axi hold` so the item carries the same lapse deadline every other captain hold does, and so a refresh keeps a deadline the clock has not reached.
