...
firstmate composed the card from that fleet state, so it already holds the routing; the card's opaque `source` blob (`{task, pr, channel}`, plus the `producer` ownership stamp on a card the board refresh composed itself) that firstmate set when pushing it is the richer route, readable from the board's own read API (`/api/board`) if the `item_id` prefix is ever insufficient.
...
- **A ready-action card** (`kind:"action"`, `value:"merge"`) - the captain approved shipping. Merge the PR with `bin/fm-pr-merge.sh <task-id> <full GitHub PR URL>`, taking the URL from the card's own `source.pr`: review-ready work is often already torn down, so `state/<id>.meta` and its recorded `pr=` may be gone by the time the captain answers. Continue that task's normal teardown flow only while `state/<id>.meta` still exists. Without one there is nothing left to tear down - `bin/fm-teardown.sh` refuses outright without a meta - and nothing left owed: the merge itself closed the backlog item and refreshed the project clone, which is the bookkeeping teardown would have done, and warned on stderr about any of it that it could not do (a close that failed, or a backlog it must not write because `config/backlog-backend=manual` leaves it to hand-editing) - finish that part by hand, or the next sync recomposes the very card you just answered. `value:"hold"`/`"dismiss"`/`"skip"`/etc. means do not merge; just clear the card. A `+captain-merge` project's action card carries `dismiss` in place of `merge` because firstmate must never merge it (`AGENTS.md` section 6) - the captain merges it themselves, and the card is only theirs to clear; should a `merge` answer for one reach you anyway from a stale or replayed card, `bin/fm-pr-merge.sh` refuses it, which is the correct outcome and not a failure to work around. This rule is `kind:"action"` only: firstmate offers a Merge option only on an action card, and only after verifying the PR names the owner/repo the task's project actually pushes to, so an `action` card's `source.pr` is a checked url. A `decision` card carries no Merge option and its `source.pr` is unverified, so a free-text "merge" there is a captain instruction to route through the free-text rule below - reconcile it against the task's live state first, never straight into `fm-pr-merge.sh`.
  Answering `dismiss` clears the card without closing the durable backlog item, so compose keeps deriving it - but the clear is recorded as settled, so neither the mid-session board refresh nor the next session-start sync re-declares that card while its composed content is unchanged. It comes back only once the situation actually moves, which is a new question rather than the one you dismissed (`docs/configuration.md` "Board refresh" owns that record).
