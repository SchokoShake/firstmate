...
- **A ready-action card** (`kind:"action"`/`decision`, `value:"merge"`) - the captain approved shipping. Merge the PR with `bin/fm-pr-merge.sh <task-id> <full GitHub PR URL>` (the recorded `pr=` in `state/<id>.meta` is the URL), then continue that task's normal teardown flow. `value:"hold"`/`"skip"`/etc. means do not merge; just clear the card.
