...
- **A ready-action card** (`kind:"action"`/`decision`, `value:"merge"`) - the captain approved shipping. Merge the PR with `bin/fm-pr-merge.sh <task-id> <full GitHub PR URL>`, taking the URL from the card's own `source.pr`: review-ready work is often already torn down, so `state/<id>.meta` and its recorded `pr=` may be gone by the time the captain answers. Then continue that task's normal teardown flow. `value:"hold"`/`"skip"`/etc. means do not merge; just clear the card.
