...
- `FLEET_SYNC: <name>: skipped: not a git repo` - the one skip reason that is drift rather than a one-off: a directory under `projects/` that is not a clone at all, such as a scratch dir or an interrupted clone.
  It repeats at every session start until that directory becomes a real clone or is removed; the line itself cannot be acknowledged or dismissed - it is deliberately left unfiltered so the anomaly stays visible - so read it as a project the captain believes they have and does not; nothing was refreshed in its place.
...
- `LOGBOOK: on - board at <url>` - the opt-in attention board is enabled and bootstrap ensured its server is up (`docs/configuration.md` "Logbook (config/logbook.env)"); a capability fact, not a problem.
  ...
  It also reports `board refresh armed ...`, which keeps the board's CONTENTS current as work is dispatched, finished, and decided (`docs/configuration.md` "Board refresh"); it is silent on every path and never wakes you, and because it only ever rewrites or clears the mechanical baseline it composed itself, a card you pushed by hand still needs your own resolve.
