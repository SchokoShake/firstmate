...
- `MISSING: <tool> (install: <command>)` - list the missing tools to the captain with a one-line purpose each plus the printed install commands, wait for consent (one approval may cover the list), then run `bin/fm-bootstrap.sh install <approved tools...>`.
  ...
  For `no-mistakes` and `gh`, this also covers an installed version below the floor [`bin/fm-bootstrap.sh`](../../../bin/fm-bootstrap.sh) owns, so PRs carry visible screenshots and crewmate validation briefs get no-mistakes' version-matched gate guidance.
  An outdated no-mistakes prints `no-mistakes update` as its install command, which restarts the shared daemon and refuses while any validation run is active; that refusal changes nothing, so retry when the fleet is quiet.
...
- `FLEET_SYNC: <repo>: skipped: not a git repo` - the one skip reason that is real drift rather than a benign one-off: a directory under `projects/` that is not a clone, usually a scratch dir or a half-finished clone.
  It is worth surfacing because it is a project the captain believes they have, so report it rather than dismissing it; the clone gate is bounded at `projects/`, so this line is accurate and cannot be waved through by the enclosing repository answering in the directory's place.
...
- `LOGBOOK_NOTIFY: answer nudge unavailable - could not publish state/logbook-notify-command` / `LOGBOOK_NOTIFY: answer nudge off - could not remove stale state/logbook-notify-command` - bootstrap could not write or clear the board's notify-command artifact (`docs/configuration.md` "Board wake-ups"), so no board-answer nudge reaches this session.
  The board poll still drains every answer, so this costs the poll's latency and never a lost or duplicated answer; make `state/` writable and the artifact path a plain file or absent, then rerun `bin/fm-bootstrap.sh`.
