...
# `#{pane_current_path}` probe from FM_FAKE_PANE_PATH, and installs the shared
# treehouse lease stub that hands out that same path, so this same fakebin
# works for a crew/scout (non-secondmate) spawn's durable lease and worktree
# wait loop.
