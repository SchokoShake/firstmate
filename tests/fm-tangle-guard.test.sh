...
# A fake tmux that reports FM_FAKE_PANE_PATH as the pane cwd once spawn has cd'd
# into its leased worktree (fm_fake_treehouse leases that same path, so the
# spawn's worktree-resolution loop resolves to a path we control), names
...
#     are disabled so the fm-<id> name survives the pane's cd into the worktree;
...
test_spawn_tmux_window_construction() {
  ...
  # Bug 2 fix (b): the move into the leased worktree and the worktree wait loop
