...
3. **Never tear down a worktree that holds unlanded work.**
   ...
   Work counts as "landed" when: `HEAD` is reachable from any remote-tracking branch (a fork counts, so an upstream-contribution PR pushed to a fork satisfies this in any mode); or, for a normal ship task, its PR merged with a head that contains the local work, its PR merged into the default branch (the normal post-merge teardown of a pipeline-rebased branch), or its content is already present in the up-to-date default branch; or, for `local-only` ship tasks with no remote, it merged into the local default branch.
   ...
   The full per-clause mechanics and the `pr=` discovery fallback are owned by `bin/fm-teardown.sh`'s header, not restated here.
...
config/logbook-mode.env  generated logbook watcher cadence (exports FM_CHECK_INTERVAL=15); LOCAL, gitignored; a distinct GENERATED file, never the hand-authored config/logbook.env; source before arming the watcher when present (section 15)
...
2. **Bootstrap** - detect-only diagnostics (tool/version problems, GitHub auth, the worktree-tangle check, harness override, dispatch-profile validation, backlog-backend status) always run and always print.
   ...
   The five MUTATING sweeps - fleet sync, the local secondmate fast-forward sweep, the secondmate liveness sweep, X-mode artifact writes, and the logbook board-server launch - run only when this session actually holds the lock from step 1.
...
Run `bin/fm-pr-check.sh <id> <PR url>` - it records `pr=` and GitHub's `pr_head=` when available in the task's meta, records the PR on the backlog item too so it survives teardown, and arms the watcher's merge poll.
That durable half never fails the arm, but it never hides either: heed any warning it prints about a record it could not make, because a PR the item never carries is one the board loses the moment the crew is torn down - record it by hand instead (section 10).
...
`bin/fm-teardown.sh`'s header owns the full landed-work definition (remote-reachable, merged-PR-head containment for the squash-merge-then-delete-branch flow, a PR merged into the default branch for the pipeline-rebased post-merge teardown, content already in the default branch, local-only merges) and the `pr=` discovery fallback for merges that skipped `bin/fm-pr-check.sh`.
...
When logbook is enabled, every item that reaches the captain here - a decision to make, an action ready to take, or an FYI - also hits the attention board promptly, the moment it arises, in addition to chat (push it with `bin/fm-logbook-push.sh`, resolve it once acted on; section 15); when logbook is off, chat is the only surface.
...
Captain-held cards on the attention board (section 15) are a tasks-axi-backend capability: the hold the board reads is the one `tasks-axi hold` writes, so under `manual` a hand-maintaining captain gets no card for a captain-gated thread.
...
- Record a task's PR while it is still open: `tasks-axi update <id> --pr <url>`.
  `bin/fm-pr-check.sh` already does this for you at the PR-ready stage (section 7), so this is the by-hand form for a backlog item whose PR was never armed there.
  The durable record matters because `state/<id>.meta` dies with the crew while the task lives on waiting for review.
...
- `logbook-respond` - load on a `logbook-response <response_id>` `check:` wake to act on the captain's board answer through the normal lifecycle, and on a `logbook-error ...` `check:` wake to report the logbook configuration blocker; relevant only when logbook is on.
