...
- `LOGBOOK: on - board at <url>` - the opt-in logbook attention board is enabled and bootstrap ensured its server is up (a trailing `(server not reachable yet; see the diagnostic above)` means the launch is still settling or a launcher diagnostic printed just above); a capability fact, not a problem. On a reachable board bootstrap also emits a captain-facing `LOGBOOK: attention board: <url>` line and auto-syncs the board from fleet state, so relay that link to the captain once at session start (and after a restart) and do not hand-run the session-start sync yourself (section 15). A following `LOGBOOK: board-response poll armed ...` line means the Phase 2 inbound answer-loop is wired (the poll shim + 15s cadence); apply the cadence to a running watcher with a sourced `--restart` per section 15. `LOGBOOK: off - removed board-response poll shim ...` reports an opt-out that cleared those artifacts. Feed and reconcile the board per section 15; steady-state off prints nothing.
