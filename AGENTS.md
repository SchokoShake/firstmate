...
Run `bin/fm-pr-check.sh <id> <PR url>` - it records `pr=` and the forge's `pr_head=` when available in the task's meta, records the same URL on the backlog item, and arms the watcher's merge poll.
