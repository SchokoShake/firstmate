...
Its ready signal is `done: stack ok: ...` instead; record the stack with every PR bottom to top through `bin/fm-pr-check.sh`, which records nothing unless `bin/fm-stack-check.sh` proves GitHub reports exactly that stack.
