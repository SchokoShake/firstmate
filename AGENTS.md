...
state/               runtime records and signals; gitignored
  ...
  board-session.json this session's own identity (session id, session pid), published only by the lock holder so a board can wake it without a hand-set session name; bin/fm-session-start.sh
  primary-inbox      this session's cross-session inbox, published only by the lock holder; lets a board nudge the running session so a captain's answer is drained at once instead of at the next poll. Notification only, never answer content; bin/fm-inbox-post.sh
  logbook-notify-command  generated command line a board spawns to send that nudge; present only while a board poll is armed
...
2. **Bootstrap** - detect-only checks (tool/version problems, the worktree-tangle check, harness override, dispatch-profile validation, backlog-backend status) always run, but routine confirmations stay silent by default.
   ...
   Home-local stale Herdr projection cleanup and the seven bootstrap MUTATING sweeps - non-executing legacy PR-check migration, fleet sync, secondmate convergence, secondmate liveness, pending remote handoff retry, Relay artifact writes, and the logbook notify-command write - run only when this session actually holds the lock from step 1; the four network ones among them run in the deferred stage rather than in this section.
...
Pass `--base <branch>` to that spawn whenever the brief's Setup line branches the task from anything other than the default branch, so `base=` is recorded for the board's branch tree (contract: `bin/fm-spawn.sh` header).
...
Its header owns how each run step and outcome maps to a crew state; parked approval or fix-review states require the worker to follow the active gate help.
...
Run `bin/fm-pr-check.sh <id> <PR url>` - it records `pr=` and, when the forge supplies them, the `pr_head=` and `pr_base=` refs in the task's meta, records the same URL on the backlog item, and arms the watcher's merge poll.
Dependent work that ships as a chain is one native GitHub stack scaffolded with `bin/fm-brief.sh --stack`, never PRs whose bases merely chain.
Its ready signal is `done: stack ok: ...` instead; record the stack with every PR bottom to top through `bin/fm-pr-check.sh`, which records nothing unless `bin/fm-stack-check.sh` proves GitHub reports exactly that stack, and leave merging to the captain, because firstmate never merges a stacked PR.
...
Relay or an enabled logbook board may require that same live cycle with no fleet work.
