...
| `fm-ask.sh`              | Read a captain-held row's question identity and revision, and deliberately re-ask it with `again`, the only writer of `data/ask-revisions` |
...
| `fm-project-mode.sh`     | Resolve a project's registered delivery posture and `+captain-merge` policy from `data/projects.md` |
...
| `fm-cadence-lib.sh`      | Single owner of watcher cadence carriers: which files count, per-home resolution, snappiest-wins order, and read-by-pattern application |
...
| `fm-supervision-lib.sh`  | Single owner of the supervision-need predicate and the fresh-watcher-beacon check     |
| `fm-ff-lib.sh`           | Shared guarded fast-forward helper for origin pulls and local secondmate syncs       |
| `fm-lock-lib.sh`         | Shared "is this git lock provably abandoned?" proof used by teardown and fleet-sync   |
| `fm-config-inherit-lib.sh` | Shared primary-to-secondmate inherited local-material propagation and config-reread delivery |
| `fm-tasks-axi-lib.sh`    | Shared backlog-backend selector, `tasks-axi` compatibility probe, and single decoder of a `tasks-axi show --full` field |
...
| `fm-teardown.sh`         | Fail-closed teardown: return landed ship worktrees, require completed scout deliverables, retire secondmate homes, and retire only the record of a task whose pool slot was re-leased |
