...
- **A surface can still die in the brief window between `target_ready` succeeding and the operation's own call running.** That remaining race degrades to "the operation quietly did nothing" - the same class of gap firstmate already tolerates for an unverified send on any backend, caught downstream by `fm_backend_cmux_send_text_submit`'s retry loop (which reports `send-failed`/`pending`/`unknown` rather than a false "sent"), or the watcher's stale-pane detection. One of those downstream catches has since been removed: `fm-spawn.sh` used to infer a dropped spawn-time send from its worktree-discovery poll timing out, but it now leases the worktree itself and never reads a surface's cwd back (see [`docs/tmux-backend.md`](tmux-backend.md) "Worktree capture"), so that backstop is gone. A later spawn-time send into a surface that stayed dead still fails its own `target_ready` check and aborts the spawn; the ship brief's opening isolation assertion and the watcher's stale-pane detection remain the backstops for a send that was merely dropped.
