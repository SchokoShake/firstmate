...
`bin/fm-supervision-lib.sh` also recognizes a Relay poll and an enabled logbook board as supervision need, but unaccounted primary work still contributes nothing to that predicate.
Without an independent Relay or logbook need, unaccounted primary work therefore reads as idle rather than suspicious.
