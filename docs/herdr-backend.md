...
The replaced tab is only half of a respawn's idempotency; the crew's worktree is the other half, and it is owned by `fm-spawn.sh`, not this adapter.
A respawn reuses the `worktree=` already recorded in `state/<id>.meta` instead of leasing a second one, so the replacement tab lands back in the same worktree with the crew's branch, commits, and uncommitted work intact ([`docs/tmux-backend.md`](tmux-backend.md) "Respawn: reuse the recorded worktree, never lease a second").
...
- `pane get`'s `.result.pane.cwd` field is frozen at pane-creation time and never updates; the adapter's cwd read originally used it and would have made `fm-spawn.sh`'s then-current worktree-discovery poll misresolve the acquired treehouse worktree path (it would see the pane's ORIGINAL directory, not where `treehouse get`'s subshell actually landed) - fixed at the time by reading `.result.pane.foreground_cwd` instead, which tracks the live running process. Moot since: `fm-spawn.sh` leases the worktree itself rather than reading a pane's cwd back (`docs/tmux-backend.md` "Worktree capture"), so that adapter op has been removed. The frozen-`cwd` finding is kept here because it is still a trap for any future caller tempted to read the field.
...
**Invisible space padding (added 2026-08-12, fleet-wide).** A harness may pad an otherwise-empty composer row with an INVISIBLE Unicode space instead of ASCII `0x20`, and bash's `[[:space:]]` is ASCII-only under `C.UTF-8`, so every adapter's own trim left that padding behind and an empty composer classified as `pending`.
Claude 2.1.228 does exactly that (`❯` followed by U+00A0 NO-BREAK SPACE; the `harness-adapters` skill owns that per-harness quirk and its live evidence), which made `bin/fm-send.sh` report a swallowed Enter on steers it had in fact delivered and made every idle claude pane read as pending input to the away-mode injector.
`fm_composer_classify_content` therefore folds invisible Unicode spaces to ASCII and re-trims before judging the row, repairing all four adapters at the single point they delegate to; `bin/fm-composer-lib.sh` owns the exact folded character set and why visible `Zs` characters such as U+1680 stay out of it.
Only invisible padding folds, so the fold can reclassify a row that was already visually blank but never one showing real text: genuinely unsubmitted text still reads `pending`, still defers injection, and still exits `fm-send.sh` non-zero.

**Regression coverage.** `tests/fm-composer-lib.test.sh` pins the shared owner directly (bare shell glyph -> `unknown`, the same glyph bordered -> `empty`, agent glyphs -> `empty` bordered or bare, idle placeholder, real text -> `pending`), and pins the space fold plus its non-weakening of those `pending` and dead-shell `unknown` rules.
`tests/fm-composer-ghost.test.sh` pins four live-captured claude 2.1.228 composer rows byte-for-byte: the three delivered-send shapes that must read `empty` (idle, mid-turn with an empty composer, and a mid-turn send accepted as a queued message) and the stranded row that must still read `pending`.
`tests/fm-send-strict.test.sh` pins `fm-send.sh`'s exit code in both directions: 0 for a delivered or queued send, non-zero for a genuinely stranded one.
...
- **RESOLVED: worktree-discovery isolation guard's symlinked-project-prefix false refusal.** Originally discovered while building the runtime-backend-auto-detection real smoke test (`tests/fm-backend-autodetect-smoke.test.sh`), which needed a scratch project.
  ...
  Update 2026-07-15 (`fm-spawn-wt-batch-x5`): the pane-current-path worktree-discovery poll this record describes was later removed - `fm-spawn.sh` now captures the worktree authoritatively from `treehouse get --lease` and `cd`s the pane into it (see [`docs/tmux-backend.md`](tmux-backend.md) "Worktree capture"). `PROJ_ABS_REAL` remains load-bearing but is now used only by `validate_spawn_worktree`'s primary-vs-worktree comparison, and `test_spawn_symlinked_project_prefix_avoids_false_refusal` was reframed to drive that comparison via a leased worktree rather than a two-reply pane poll.
