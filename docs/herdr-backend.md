...
Task cleanup acquires that session lock before the pane is closed or the task's isolated copy is returned, so a contended lock refuses up front while the copy, every durable record, and the endpoint are all intact for a plain rerun.
It closes the pane before returning the copy, because the pane's own shell sits inside that copy and returning first would end the shell outside the focus-safe plan; its process cleanup spares exactly that shell for the same reason.
...
The isolated copy is returned and durable task records are erased only once the exact pane is confirmed gone through its structured presence: after every close path, only a structured not-found response counts as gone, while a present or unknown result retains the copy and every record with a visible, retryable error.
