...
Three of firstmate's contracts are read a second time by code firstmate does not own: the AGENTS.md section 10 backlog item line, which the logbook connector re-reads through `tasks-axi` and its own title and id rules; the PR slug and same-repo-part parse in `bin/fm-merge-policy-lib.sh`, which that connector re-implements to decide whether a repo's Merge control may be offered; and the `base` / `pr_base` pair on a `fm-fleet-snapshot.v1` task row, from which the logbook board builds its branch tree - each branch a child of the branch it merges into - without a per-PR forge call inside its refresh loop.
None can be consolidated, because the two implementations live in different repositories, and every way they drift fails silently: a merge slips past a guard that could not read its url, a row the captain wrote never reaches the board, or a workstream is parented under the wrong branch.
...
`tests/fixtures/pr-slug/`, `tests/fixtures/backlog-item-line/` and `tests/fixtures/snapshot-branch-parent/` are those statements, pinned on this side by `tests/fm-pr-slug-contract.test.sh`, `tests/fm-backlog-item-line-contract.test.sh` and `tests/fm-snapshot-branch-parent-contract.test.sh`.
