...
Absorbing a declared wait follows the authoritative verdict itself and does not additionally require a dead agent, because an idle pane is what a declared wait looks like while its worker is alive and waiting.
`crew_absorb_class`'s done-reconciliation rule extends that cadence to a crew whose attributed run reports `done` while its status log's last line still declares the wait, such as one holding its own open PR until the captain merges it; that one rule reconciles `done` alone, because a `parked` gate and a `failed` run both need firstmate.
The watcher's per-window recheck cache is a separate and wider bound: once a window whose last status line declares a wait has surfaced a stale wake, the cache replays `paused` for that window regardless of the verdict just classed, so even a live crew parked at a gate re-surfaces only once per `FM_STALE_ESCALATE_SECS`, the same cadence as the wedge timer.
An authoritative `working` verdict returns the window to the `FM_STALE_ESCALATE_SECS` wedge timer.
A backend that confidently reports the agent dead restores the declared-wait cadence for an unchanged `paused:` or durable `captain-held` endpoint once `bin/fm-crew-state.sh` reports a state that absorbs nothing on its own, typically `unknown` after the endpoint reads as a bare shell.
...
`bin/fm-crew-state.sh <id>` is the cheap current-state read for an actionable heartbeat review: it considers only the newest no-mistakes run on the crew's branch, active or terminal, attributes it only when it matches the crew's current code identity, then keeps that run-step authoritative even if the pane has closed.
An older run on the same branch never stands in for the newest one, so the superseded failure of a resumed validation cannot be reported as the task's outcome.
The script header owns the exact attribution rules.
...
A pull-based guard (`bin/fm-guard.sh`) warns through supervision tool output if the primary checkout is tangled, if work, process-event sources, Relay polling, or an enabled logbook board has an unhealthy model-aware supervision verdict, or if queued wakes are waiting to be drained.
...
On every verified primary harness, tracked hook integration gives the primary session a push-based backstop: when work, a process-event source, Relay polling, or an enabled logbook board needs supervision and no identity-matched watcher lock with a fresh beacon is live, blocking-capable Stop hooks block and nonblocking turn-end integrations force one bounded follow-up.
...
Dependent work that ships as a chain uses the `direct-PR` stacked-chain variant, `bin/fm-brief.sh --stack`, whose done gate is a native GitHub stack proven by `bin/fm-stack-check.sh`; `bin/fm-pr-check.sh` records such a task only with that proof, and each script's header owns its contract.
...
A registry line's optional `+captain-merge` flag is read through that same parser and enforced by `bin/fm-merge-policy-lib.sh` on both of firstmate's merge paths, from the task's own recorded project and, independently, from the PR url traced to a clone's origin.
...
It refuses just the same a record with a recorded claim whose copy the pool durably leases to a holder other than that claim, which is the pool's own recorded fact and no inference about ownership.
...
Four of firstmate's contracts are read a second time by code firstmate does not own: the AGENTS.md section 10 backlog item line, which the logbook connector re-reads through `tasks-axi` and its own title and id rules; the PR slug and same-repo-part parse in `bin/fm-merge-policy-lib.sh`, which that connector re-implements to decide whether a repo's Merge control may be offered; the `base` / `pr_base` pair on a `fm-fleet-snapshot.v1` task row, from which the logbook board builds its branch tree - each branch a child of the branch it merges into - without a per-PR forge call inside its refresh loop; and `state/board-session.json`, the session identity a board reads to wake this session by session id.
None can be consolidated, because the two implementations live in different repositories, and every way they drift fails silently: a merge slips past a guard that could not read its url, a row the captain wrote never reaches the board, a workstream is parented under the wrong branch, or a wake is aimed at a session that is not there.
...
`tests/fixtures/pr-slug/`, `tests/fixtures/backlog-item-line/`, `tests/fixtures/snapshot-branch-parent/` and `tests/fixtures/board-session/` are those statements, pinned on this side by `tests/fm-pr-slug-contract.test.sh`, `tests/fm-backlog-item-line-contract.test.sh`, `tests/fm-snapshot-branch-parent-contract.test.sh` and `tests/fm-board-session.test.sh`.
