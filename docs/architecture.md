...
The session-start reconcile is automatic: bootstrap runs `bin/fm-logbook-refresh.sh`, which composes the attention set from fleet state with `bin/fm-logbook-compose.sh` (a mechanical `{projects, items}` baseline - one card per in-flight task, per PR-ready task, and per open needs-decision, projects flagged active where they carry a card - from `data/projects.md`, `data/backlog.md`, `state/*.meta`, and `state/*.status`) and hands it to `bin/fm-logbook-sync.sh` (`POST /api/sync`, a declarative truth-restore that stays accurate across restarts); firstmate re-runs `bin/fm-logbook-refresh.sh` to re-truth the board mid-session. It then upserts richer changed items with `bin/fm-logbook-push.sh` (`POST /api/items`) and clears acted-on cards with `bin/fm-logbook-resolve.sh`, which - the tool having no dedicated resolve endpoint and rejecting a bare `{id, status}` upsert - reads the card's current fields via `GET /api/board` and re-upserts the whole item with a terminal status (an id absent from the board is a clean no-op).
