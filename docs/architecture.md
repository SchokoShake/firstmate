...
On the locked session-start bootstrap step, a truthy `LOGBOOK_ENABLE` makes bootstrap ensure the board server is up through `bin/fm-logbook-up.sh` (detached and non-blocking), print `LOGBOOK: on - board at <url>`, and arm the inbound poll shim plus cadence file (needing `curl`+`jq`); on opt-out it removes those artifacts, and with no config and no leftover artifacts it is a complete no-op, so non-adopters see no behavior change.
...
`LOGBOOK_DRY_RUN` previews the push, sync, resolve, and ack bodies under `state/logbook-outbox/` without posting (push, sync, and ack need no token or board, while resolve still performs its read-only `GET /api/board` to compose the full item it would upsert; the read-only poll needs no such switch).
