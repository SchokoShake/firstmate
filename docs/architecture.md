...
`LOGBOOK_DRY_RUN` previews the push, sync, resolve, and ack bodies under `state/logbook-outbox/` without posting or a token (the read-only poll needs no such switch).
