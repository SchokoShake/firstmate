...
For backends that do not create the worktree themselves, `fm-spawn.sh` leases the crew worktree authoritatively with `treehouse get --lease` (its stdout is the source of truth) and sends the backend a `cd` into that path, instead of polling the pane's current path: a pane-cwd read transiently returned the session server's own start dir (`$FM_HOME`) in the moment right after window creation, mis-recording it as the worktree on projects/* spawns (`bin/fm-spawn.sh`; fm-spawn-wt-batch-x5).
...
`data/projects.md` records each project's delivery mode, optional `+yolo` autonomy flag, and optional `+captain-merge` flag for a project firstmate must never merge.
...
That recording serves a later teardown, so a task whose meta teardown has already swept merges from the PR URL alone and closes its own backlog item in that absent teardown's place; the script's header owns why.
...
As of Phase 2 the loop is two-way: firstmate pushes and reconciles the board, and the captain can answer a card on the board (or still in chat), after which firstmate acts on the answer and resolves the card. The inbound answer-loop rides the existing check mechanism exactly as X mode does - a poll shim `state/logbook-watch.check.sh` -> `bin/fm-logbook-poll.sh`, a generated `config/logbook-mode.env` cadence (`FM_CHECK_INTERVAL=15`), and the `logbook-respond` skill - without editing the watcher backbone.
On the locked session-start bootstrap step, a truthy `LOGBOOK_ENABLE` makes bootstrap ensure the board server is up through `bin/fm-logbook-up.sh` (detached and non-blocking), print `LOGBOOK: on - board at <url>`, and arm the inbound poll shim, the board-liveness reap shim, the board-refresh shim, and the cadence file (needing `curl`+`jq`); when the board is reachable it also prints a captain-facing `LOGBOOK: attention board: <url>` link and auto-syncs the board from fleet state via `bin/fm-logbook-refresh.sh` (best-effort and non-blocking). On opt-out it removes those artifacts, and with no config and no leftover artifacts it is a complete no-op, so non-adopters see no behavior change.
...
The board is a bare detached process with no supervisor of its own, so a second check shim `state/logbook-reap.check.sh` -> `bin/fm-logbook-reap.sh` health-checks it on every watcher cycle and relaunches a board killed mid-session through that same idempotent `bin/fm-logbook-up.sh`, staying silent unless it gives up relaunching (then one `logbook-error` wake); where systemd exists a `systemd --user` unit with `Restart=always` is the recommended supervisor instead, and `docs/configuration.md` "Board liveness" owns the full contract.
The session-start reconcile is automatic: bootstrap runs `bin/fm-logbook-refresh.sh`, which composes the attention set from fleet state with `bin/fm-logbook-compose.sh` (a mechanical `{projects, items}` baseline from `data/projects.md`, `data/backlog.md`, `state/*.meta`, `state/*.status`, and each project clone's `origin` remote - compose's only reach into `projects/`, a read-only `git remote get-url` that gates a card's Merge on the PR naming the repo the project really pushes to - projects flagged active where they carry a card) and hands it to `bin/fm-logbook-sync.sh` (`POST /api/sync`, a declarative truth-restore that stays accurate across restarts); firstmate re-runs `bin/fm-logbook-refresh.sh` to re-truth the board mid-session.
That sync alone left the board a photograph of session start, so a third check shim `state/logbook-resync.check.sh` -> `bin/fm-logbook-resync.sh` re-composes on every watcher cycle whose cheap content fingerprint of compose's own inputs has moved, and reconciles INCREMENTALLY - never through `/api/sync`, whose full-ITEM replace would delete every hand-pushed card and flatten every rich one on a 15s beat, but by adding, correcting, and clearing only the cards carrying compose's own ownership stamp, always silently; `docs/configuration.md` "Board refresh" owns the full contract.
The item SET comes from the durable backlog - In flight, plus any captain-held task in any state - never from live crew runtime, because `state/<id>.meta` exists only while a crew runs while the documented end-state for review-ready work is teardown plus a captain hold; a meta only ENRICHES a backlog-derived item (the live crew's recorded PR, base branch, and phase), and adds one of its own only for a running crew the backlog has not recorded yet. `bin/fm-logbook-compose.sh`'s header owns that rule in full. It then upserts richer changed items with `bin/fm-logbook-push.sh` (`POST /api/items`) and clears acted-on cards with `bin/fm-logbook-resolve.sh`, which - the tool having no dedicated resolve endpoint and rejecting a bare `{id, status}` upsert - reads the card's current fields via `GET /api/board` and re-upserts the whole item with a terminal status (an id absent from the board is a clean no-op).
The PR a torn-down task's card links, and gates its Merge option on, is the durable one `bin/fm-pr-check.sh` records on the backlog item at the same PR-ready moment it arms the merge poll; where several were recorded, the newest wins, matching the last-wins `pr=` read from a live meta.
Work the backlog itself records as not-ready - an active hold that is not a captain hold, or a `blocked-by: <id>` dependency whose blocker has not finished - still carries that link but is never offered the Merge, and each marker is read as the state it describes (a lapsed `hold-until:` gate, a landed blocker) rather than as text nothing ever rewrites.
A `+captain-merge` project's card is withheld the Merge too, however ready and verified its PR; the [logbook configuration reference](configuration.md#logbook-configlogbookenv) owns that contract and its enforcement.
The inbound answer-loop closes the same way: `bin/fm-logbook-poll.sh` polls `GET /api/connector/poll`, stashes each pending answer to `state/logbook-inbox/`, and prints `logbook-response <id>`; on that `check:` wake the `logbook-respond` skill drains the inbox, applies each answer through the normal lifecycle, then acks it with `bin/fm-logbook-ack.sh` (`POST /api/connector/ack`) and resolves the card.
...
`LOGBOOK_DRY_RUN` previews the push, sync, resolve, and ack bodies under `state/logbook-outbox/` without posting (push, sync, and ack need no token or board, while resolve still performs its read-only `GET /api/board` to compose the full item it would upsert; `refresh` inherits the switch through its sync step and `resync` through the upserts and clears it makes, and the read-only `poll` and `compose` need no such switch).
The watcher, wake queue, arm wrapper, and afk daemon are untouched; logbook lives entirely in the `bin/fm-logbook-*.sh` scripts, the `logbook-respond` skill, and bootstrap's opt-in `logbook_setup` block.
