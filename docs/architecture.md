...
On the locked session-start bootstrap step, a truthy `LOGBOOK_ENABLE` makes bootstrap ensure the board server is up through `bin/fm-logbook-up.sh` (detached and non-blocking), print `LOGBOOK: on - board at <url>`, and arm the inbound poll shim plus cadence file (needing `curl`+`jq`); when the board is reachable it also prints a captain-facing `LOGBOOK: attention board: <url>` link and auto-syncs the board from fleet state via `bin/fm-logbook-refresh.sh` (best-effort and non-blocking). On opt-out it removes those artifacts, and with no config and no leftover artifacts it is a complete no-op, so non-adopters see no behavior change.
