...
`LOGBOOK_DRY_RUN` previews the push, sync, resolve, and ack bodies under `state/logbook-outbox/` without posting (push, sync, and ack need no token or board, while resolve still performs its read-only `GET /api/board` to compose the full item it would upsert; `refresh` inherits the switch through its sync step, and the read-only `poll` and `compose` need no such switch).
