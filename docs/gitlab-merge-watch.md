...
A GitLab task records no `pr_head=` and no `pr_base=`.
[`bin/fm-pr-check.sh`](../bin/fm-pr-check.sh)'s header owns why plain `glab` cannot supply either field and which consumers treat each as optional.
