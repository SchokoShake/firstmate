...
`pane_cwd` follows a top-level shell `cd`, which is how spawn now moves a pane into its leased worktree, but not a `cd` made inside a nested foreground subshell.
Worktree discovery still sends begin and end markers around `pwd`, captures the marked block, and joins wrapped path lines, so it reads the live shell's cwd however the pane got there.
