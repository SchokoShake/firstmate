...
One of those downstream catches has since been removed: `fm-spawn.sh` used to infer a dropped spawn-time send from its worktree-discovery poll timing out after 60s, but it now leases the worktree itself and never reads a pane cwd back (see [`docs/tmux-backend.md`](tmux-backend.md) "Worktree capture"), so that particular 60s backstop is gone.
A later spawn-time send into a pane that stayed dead still fails its own `target_ready` check and aborts the spawn; the ship brief's opening isolation assertion and the watcher's stale-pane detection remain the backstops for a send that was merely dropped.
...
The one real bug this pass caught - the `pane_cwd`-does-not-track-a-subshell gap (see "Known gaps left for a follow-up" below) - was found and fixed during this E2E run itself: the FIRST attempt refused to launch with "did not yield an isolated worktree" because the adapter's original JSON-only cwd read never saw `treehouse get`'s subshell move away from the project directory, so the 60-second poll's own comparison collapsed to "same path" and the isolation guard correctly (if confusingly) refused. A `pwd`-probe workaround fixed it at the time and the identical flow spawned cleanly on the very next attempt. That whole class of bug is now moot: `fm-spawn.sh` no longer discovers the worktree by reading a pane's cwd at all (`docs/tmux-backend.md` "Worktree capture"), so the probe and its adapter op have since been removed.
...
- **No live-cwd primitive exists for a zellij pane.** `list-panes --json`'s `pane_cwd` is frozen at the last foreground command's invocation time and never follows a nested subshell's own `cd`, and the CLI exposes no per-pane pid or live-cwd field to read instead. Nothing needs one today - `fm-spawn.sh` leases the worktree itself and `cd`s the pane into it rather than reading a cwd back (`docs/tmux-backend.md` "Worktree capture") - but a future caller that genuinely needs a zellij pane's live cwd would have to actively probe it (submit a marked `pwd`, capture, scrape), not read a passive JSON field.
