...
2. First cycle: arm with Grok's tracked background tool, as its own call:
   ...
   The arm applies this home's watcher-cadence carriers itself, so never source them by hand first.
...
4. Ordinary wake: re-arm the next cycle with the same background `bin/fm-watch-arm.sh` call if work remains in flight, Relay still needs polling, or a logbook board is still enabled.
