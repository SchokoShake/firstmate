...
A verified PR earns the Merge option only where firstmate is permitted to merge that project at all: a `+captain-merge` project (`AGENTS.md` section 6) composes the same action card with a `Done / dismiss` acknowledgement instead, and `bin/fm-pr-merge.sh` refuses the merge even if a stale card's `merge` answer arrives anyway - `bin/fm-merge-policy-lib.sh` owns both sides.
