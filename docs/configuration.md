...
That relay outlives the backoff above, because a fall-open permission warning suppressed on 19 beats in 20 is the same defect as one suppressed always, only smaller.
...
A skipped beat still says so on stderr, so a hand run can tell a backed-off beat from a beat with nothing to do, and it still relays the merge-policy diagnostics below; only the expensive compose is skipped.
