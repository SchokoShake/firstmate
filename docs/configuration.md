...
A backend selection change applies to future spawns; a task already recorded in `state/<id>.meta` keeps its recorded backend, and respawning that id under a changed selection is ordinary recovery as long as both backends borrow the treehouse pool - the respawn reuses the recorded `worktree=` unchanged.
Crossing the Orca boundary in either direction is the exception and is refused, because Orca owns its own worktree while every other backend borrows the pool; tear the recorded task down first to switch it deliberately (see [`docs/tmux-backend.md`](tmux-backend.md) "Respawn: reuse the recorded worktree, never lease a second").
A respawn is also refused when the recorded `project=` or `kind=` disagrees with what is being spawned, since that meta describes a task firstmate has lost track of.
...
A directory under `projects/` that is not a clone at all - a scratch dir, an interrupted clone - is skipped as `not a git repo` and is relayed, so it surfaces at every session start until that directory becomes a real clone or is removed rather than passing as a refreshed project; the line itself cannot be dismissed - it is deliberately left unfiltered so the anomaly stays visible.
...
This section is the single owner of the logbook cadence contract.
An opted-in instance polls the board every 15 seconds instead of the default 300, snappier than X mode's 30 because a board answer is a direct captain decision worth acting on quickly.
Only an opted-in instance speeds up, because a home without logbook has no `config/logbook-mode.env`, and the session-start supervision operating block includes the cadence instruction when that file exists.
The active primary-harness supervision protocol owns how that sourced cadence reaches the watcher process; `bin/fm-supervision-instructions.sh` renders it, and `bin/fm-arm-command-policy.mjs` admits `config/logbook-mode.env` as an approved setup node exactly as it admits `config/x-mode.env` (see [`docs/arm-pretool-check.md`](arm-pretool-check.md)).
Both cadence configs export `FM_CHECK_INTERVAL`, so the last one sourced wins: when X mode and logbook are both on, the emitted block sources logbook second and the snappier 15s interval is the one the watcher inherits.
Because `bin/fm-watch.sh` reads `FM_CHECK_INTERVAL` only at process start, a cadence transition - opting in while a watcher is already running, or opting out - is applied by restarting the home-scoped watcher through the emitted harness protocol; bootstrap deliberately never restarts the watcher itself.
...
That last rule needs one exception, or the content diff hands `isNewClaim` far more than it was meant to see: a live-crew `fyi` card's BODY IS the last status line, so it moves on every status write a crewmate makes - several times per task on the 15s beat - and each of those upserts would clear the "not now" on exactly the progress-noise card a captain sets aside.
The update set therefore SKIPS a card the board reports set aside whose drift is its body alone and which is an `fyi` offering no options: nothing about it is answerable, so the new body is progress text rather than a new claim.
Everything else still updates and still clears the set-aside exactly as before - a changed `kind`, `title` or `options`, and a body change on a card that IS answerable, such as a decision's question text moving - and a card the captain has not set aside keeps refreshing on every drift, because a card they are watching must keep showing progress.
A board that does not report `set_aside` at all reads as not-set-aside, so the guard can only narrow to the cards it is about.
...
That is a read of the tool's behavior, so it is pinned by a test against the board stand-in in `tests/fm-logbook.test.sh` rather than trusted to stay true on its own.
