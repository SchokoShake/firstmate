...
Composing that board reads every carded project's merge policy, so bootstrap filters the refresh's stderr and re-emits any malformed-registry-line diagnostic as `LOGBOOK: registry line malformed - <detail>`.
That sync is the earliest, most frequent, and only unattended read of the `+captain-merge` posture, and a mistyped flag is not honored as the prohibition it was meant to be, so the board would otherwise offer a one-click Merge for a project the captain reserved to themselves with the warning discarded.
