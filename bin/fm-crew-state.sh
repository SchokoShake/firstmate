...
#   2. The branch's newest no-mistakes run, active or terminal, matching this
#      crew's current code identity? `axi status` run in the worktree is
#      branch-scoped: it answers with the checked-out branch's active run, else
#      that branch's most recent run, and with another branch's run only when
#      this branch has none. That one run is the only candidate - an older run on
#      the same branch is never consulted, so a superseded failed or cancelled
#      run can never stand in for the live run of a resumed validation.
...
#      the same line of history), or no-mistakes' push provenance for that run
#      names the worktree HEAD as its submitted head (the pipeline moved the head
#      with commits the worktree does not have, or rebased it). Local work
#      committed after the run's submission, or a rewritten local branch,
#      invalidates attribution.
#      bin/fm-nm-run-lib.sh's fm_nm_status_run_matches_worktree owns that rule.
...
# code-identity attribution rule below are thin wrappers over the ONE owner in
...
# The one branch-scoped `axi status` answer is the whole lookup (header, step 2):
# another branch's run means this branch has none, and a same-branch run that
# does not match the current code leaves this crew with no run. Nothing looks
# further back through the branch's history, and no plain-text run listing is
# parsed, because the next row back is an older run that a resumed validation
# has already superseded.
if [ "$KIND" = ship ] && [ -n "$CREW_BRANCH" ] && command -v no-mistakes >/dev/null 2>&1; then
  RUN_OUT=$(nm_run axi status)
  run_branch=$(strip_quotes "$(nm_field branch)")
  if [ -n "$run_branch" ] && [ "$run_branch" = "$CREW_BRANCH" ] \
    && fm_nm_status_run_matches_worktree "$WT" "$RUN_OUT"; then
    HAVE_RUN=1
  fi
fi
