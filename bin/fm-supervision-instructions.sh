...
Any watcher cadence carrier this home has is applied by the script that launches
the watcher, so the rendered block lists carriers for visibility and never asks
for them to be sourced by hand. bin/fm-cadence-lib.sh owns that contract.
EOF
}
...
# Carrier discovery, ordering, and application all belong to
# bin/fm-cadence-lib.sh. This renderer only needs the same ordered list so the
# operating block can say which carriers are in effect.
cadence_files() {
  fm_cadence_files "$CONFIG" "$X_MODE"
}
...
render_snippet() {
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    # text, so an unquoted replacement carrying `&` would come back out as the
    # placeholder itself and emit a corrupt line.
...
if [ "$X_MODE" -eq 1 ]; then
  printf '%s%s%s\n' '- X mode: active; ' "$x_mode_env" ' sets the 30s cadence, applied for you whenever a watcher is launched.'
else
  printf '%s\n' '- X mode: inactive; use the default watcher cadence.'
fi
cadence_extra=$(cadence_files | grep -Fxv "$x_mode_env" || true)
if [ -n "$cadence_extra" ]; then
  printf '%s\n' '- Watcher cadence: extra carriers active. They are applied for you whenever a watcher is launched, snappiest interval winning; do not source them by hand:'
  printf '%s\n' "$cadence_extra" | sed 's/^/    /'
fi
ordinary_wake_line
printf '\n'
