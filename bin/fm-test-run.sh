...
family_for_basename() {
  case "$1" in
    fm-arm-pretool-check.test.sh|fm-ask.test.sh|fm-ask-user-authority.test.sh|\
    fm-board-session.test.sh|fm-busy-adapter-wiring.test.sh|\
    ...
    fm-board-session-live-e2e.test.sh|fm-inbox-post-live-e2e.test.sh|\
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-stow-cascade.sh)
      printf '%s\n' secondmate
      ;;
    bin/fm-board-session-lib.sh)
      # Its own contract suite, the session start that publishes the record, and
      # the opt-in guard for the vendor half neither of those can settle.
      printf '%s\n' pure-contract-unit
      printf '%s\n' session-bootstrap
      printf '%s\n' live-harness-optin
      ;;
    ...
    bin/fm-bearings-snapshot.sh|bin/fm-fleet-snapshot.sh|bin/fm-fleet-view.sh)
      ...
      ;;
    bin/fm-ask-lib.sh)
      # Sourced by bin/fm-ask.sh (pure-contract-unit) and by
      # bin/fm-fleet-snapshot.sh, which publishes its identity on every record.
      printf '%s\n' pure-contract-unit
      printf '%s\n' snapshot-bearings
      ;;
    ...
    bin/fm-ask.sh|bin/fm-decision-hold.sh|bin/fm-supervision*|bin/fm-transition-lib.sh|\
