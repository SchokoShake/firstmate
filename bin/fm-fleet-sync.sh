...
# dir, so an unresolvable projects/ must not decline to sync one. A ceiling that is
# not an ancestor of the target is simply inert, so those paths are unaffected.
